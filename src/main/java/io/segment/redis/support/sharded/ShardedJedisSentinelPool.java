	...
	private List<HostAndPort> initSentinels(Set<String> sentinels, final List<String> masters) {
	    ...
	    for (String masterName : masters) {
	    	...
	    	while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
		    	...
		    	if (null == master) {
		    		try {
						logger.warn("All sentinels down, cannot determine where is "
		...
		public void run() {
		    ...
		    while (running.get()) {
				...
				try {
				    jedis.subscribe(new JedisPubSub() {
						...
						public void onMessage(String channel, String message) {
						    ...
						    if (switchMasterMsg.length > 3) {
								...
								logger.info("Invalid message received on Sentinel " + host + ":" + port + " on channel +switch-master: " + message);
						    }
						}
				    }, "+switch-master");
				    ...
				    if (running.get()) {
						logger.error("Lost connection to Sentinel at " + host + ":" + port + ". Sleeping 5000ms and retrying.");
						try {
