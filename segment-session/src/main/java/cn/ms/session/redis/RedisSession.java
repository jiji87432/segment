...
import cn.ms.session.common.exception.SessionException;
import cn.ms.session.core.AbstractSession;
import cn.ms.session.core.IdWorker;
...
public class RedisSession extends AbstractSession {
	...
	public RedisSession() throws IOException {
	}

	/**
	 * @param propertiesFile properties file in classpath, default is session.properties
	 */
	public RedisSession(String propertiesFile) throws IOException {
	...
	public Boolean persist(final String id, final Map<String, Object> snapshot, final int maxInactiveInterval) {
		...
		try {
			...
			logger.error("failed to persist session(id={}, snapshot={}), cause:{}",
	...
	public Map<String, Object> loadById(String id) {
		...
		try {
			...
			logger.error("failed to load session(key={}), cause:{}", sid, Throwables.getStackTraceAsString(e));
	...
	public void deleteById(String id) {
		...
		try {
			...
			logger.error("failed to delete session(key={}) in redis,cause:{}",
					sid, Throwables.getStackTraceAsString(e));
		}
	}
	...
	public void expire(String sid, final int maxInactiveInterval) {
		...
		try {
			...
			logger.error("failed to refresh expire time session(key={}) in redis,cause:{}",
